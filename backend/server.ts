  ...
  try {
    ...
    try {
      ...
      sendStageUpdate(
        requestId,
        "completed",
        "Image stored successfully! Process completed.",
        "final"
      );
      ...
      sendSSEUpdate({
        ...
        message:
          "Failed to store image, but continuing without filestore service...",
        timestamp: new Date().toISOString(),
        status: "error",
      });
